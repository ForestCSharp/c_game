...
MprVec3 mpr_vec3_add(MprVec3 a, MprVec3 b)
{
    a.x += b.x;
    a.y += b.y;
    a.z += b.z;
...
MprVec3 mpr_vec3_sub(MprVec3 a, MprVec3 b)
{
    a.x -= b.x;
    a.y -= b.y;
    a.z -= b.z;
...
MprVec3 mpr_vec3_negate(MprVec3 v)
{
    v.x = -v.x;
    v.y = -v.y;
    v.z = -v.z;
...
MprVec3 mpr_vec3_scale(MprVec3 v, float a)
{
    v.x *= a;
    v.y *= a;
    v.z *= a;
...
float mpr_vec3_dot(MprVec3 a, MprVec3 b)
{
...
float mpr_vec3_length_squared(MprVec3 v)
{
    return v.x * v.x + v.y * v.y + v.z * v.z;
}
float mpr_vec3_length(MprVec3 v)
{
    return sqrt(mpr_vec3_length_squared(v));
}
MprVec3 mpr_vec3_normalize(MprVec3 v)
{
    float l = mpr_vec3_length(v);
    v.x /= l;
    v.y /= l;
    v.z /= l;
    return v;
}
bool mpr_vec3_is_zero(MprVec3 v)
{
...
// Given that, let's discuss how to detect the contact normal and penetration depth along that normal using MPR on two
// shapes, A and B.
//
// The easiest approach is to first determine that the origin is within A-B (the Minkowski difference). You can do this
// in 2D using the MPR2D algorithm on my website or the 3D equivalent in Game Programming Gems 7 ("XenoCollide: Complex
// Collision Made Simple").
//
// Once you've identified that the origin is within A-B, you know there's a collision. You can then continue casting the
// origin ray to the boundary of A-B. Once you hit the surface, the normal of the portal is your contact normal.
//
// You now need to find the penetration depth along this contact normal. To do that, run MPR again, using the contact
// normal as the direction of your origin ray. (You can do this by using "origin - contactNormal" as your interior
// point.) FCS TODO: v0? The distance between the origin and the surface of A-B along the direction of the contact
// normal is the penetration depth.

...
- (void)keyDown:(NSEvent*)event
{
    ...
    {
		if (key_code == KEY_LEFT_CONTROL) exit(0);
...
- (void)flagsChanged:(NSEvent*)event
{
	...
	// Left Control
    if ([event modifierFlags] & NSEventModifierFlagControl && event.keyCode == 0x3B)
    {
        global_key_states[KEY_LEFT_CONTROL] = true;
    }
    else
    {
        global_key_states[KEY_LEFT_CONTROL] = false;
    }
}

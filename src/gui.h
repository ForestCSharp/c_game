...
{
    return in_point.x >= in_rect.position.x && in_point.x <= in_rect.position.x + in_rect.size.x &&
           in_point.y >= in_rect.position.y && in_point.y <= in_rect.position.y + in_rect.size.y;
}
...
Vec2 recursive_bezier(const float t, const u32 num_points, const Vec2* in_points)
{
    ...
    case 1:
        return in_points[0];
    default:
        return vec2_lerp(t, recursive_bezier(t, num_points - 1, in_points),
                         recursive_bezier(t, num_points - 1, in_points + 1));
    }
}
                   ...
                   GuiAlignment in_alignment)
{
    ...
    if (in_bounding_rect->size.x > 0.0f)
    {
        ...
        size_t max_possible_chars =
            MIN(bounding_rect_width > 0.f ? (bounding_rect_width / (char_size + spacing)) - 1 : num_chars, num_chars);
...
bool gui_hit_test(const GuiContext* const in_context, const GuiRect in_rect)
{
    for (u32 i = 0; i < sb_count(in_context->prev_frame_state.open_windows); ++i)
    {
        ...
        const bool current_window_is_busy =
            current_window.is_moving || current_window.is_resizing || current_window.is_scrolling;
        if (in_rect.z_order < window_rect.z_order &&
            (current_window_is_busy || gui_rect_intersects_point(window_rect, in_context->frame_state.mouse_pos)))
        {
            return false;
        }
    }
                              ...
                              const bool is_active)
{
    ...
    if (in_style_args)
    {
        ...
        const GuiRect normalized_draw_rect =
            gui_rect_scale(*in_draw_rect, float_div_vec2(1.0, in_context->frame_state.screen_size));
                                    ...
                                    const char* in_label, const GuiRect* const in_rect, const bool is_active)
{
    ...
    if (clicked_state == GUI_HELD)
    {
        ...
        const float new_value = remap_clamped((current_mouse_x - in_rect->position.x) / in_rect->size.x, 0.0, 1.0,
                                              in_slider_bounds.x, in_slider_bounds.y);
        *data_ptr = new_value;
    }
...
void gui_window_begin(GuiContext* const in_context, GuiWindow* const in_window)
{
    if (in_window->is_open)
    {
        ...
        {
            const GuiRect normalized_window_rect =
                gui_rect_scale(*window_rect, float_div_vec2(1.0, in_context->frame_state.screen_size));
        ...
        GuiClickState top_bar_click_state = gui_make_button(in_context, in_window->name, &top_bar_rect, &top_bar_rect,
                                                            &top_bar_button_style, !in_window->is_resizing);
...
bool gui_window_compute_control_rect(GuiWindow* const in_window, GuiRect* out_rect)
{
    ...
    if (in_window->is_open && in_window->is_expanded)
    {
        ...
        const u32 max_visible_controls =
        ...
        float y_offset = window_pos.y + window_top_bar_height + ((window_row_height) * (float)control_index) +
                         (window_row_padding_y * (float)(control_index + 1));
...
GuiClickState gui_window_button(GuiContext* const in_context, GuiWindow* const in_window, const char* in_label)
{
    ...
    {
        out_click_state = gui_make_button(in_context, in_label, &button_rect, &button_rect, &default_button_style,
                                          !in_window->is_resizing);
    }
                                      ...
                                      const Vec2 in_slider_bounds, const char* in_label)
{
    ...
    if (gui_window_compute_control_rect(in_window, &button_rect))
    {
        out_click_state = gui_make_slider_float(in_context, data_ptr, in_slider_bounds, in_label, &button_rect,
                                                !in_window->is_resizing);
    }
    ...
    if (in_window->is_open)
    {
        ...
        if (in_window->is_expanded)
        {
            ...
            { // Scrollbar FCS TODO: only draw if we need to be able to scroll
                ...
                const u32 max_visible_controls =
                    (window_rect->size.y - window_top_bar_height) / (window_row_height + window_row_padding_y);
                const float max_scroll_amount =
                    (float)MAX(0, in_window->num_controls - (window_allow_overscroll ? 1 : max_visible_controls));
                ...
                const float scrollbar_y_max = window_rect->position.y + window_rect->size.y - window_scrollbar_height -
                                              window_resize_control_size.y;
                const float scrollbar_y =
                    remap_clamped(in_window->scroll_amount, 0.0, max_scroll_amount, scrollbar_y_min, scrollbar_y_max);
                ...
                GuiRect hit_rect = in_window->is_scrolling ? gui_make_fullscreen_rect(in_context, window_rect->z_order)
                                                           : scrollbar_draw_rect;
                GuiClickState scrollbar_click_state =
                    gui_make_button(in_context, "", &scrollbar_draw_rect, &hit_rect, &default_button_style, true);
                ...
                if (in_window->is_scrolling)
                {
                    ...
                    in_window->scroll_amount =
                        remap_clamped(new_scrollbar_y, scrollbar_y_min, scrollbar_y_max, 0.0, max_scroll_amount);
                }
            }
            ...
            {
                const Vec2 resize_control_position =
                    vec2_sub(vec2_add(window_rect->position, window_rect->size), window_resize_control_size);
                ...
                in_window->is_resizing = gui_make_button(in_context, "", &resize_control_rect, &resize_control_rect,
                                                         &default_button_style, !in_window->is_moving) == GUI_HELD;

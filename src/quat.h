...
//TODO: quat_inverse: If the quaternion q = <w, v> is a unit quaternion, then its inverse is just its conjugate: q{-1} = <w, -v> (just negate x,y,z)
...
Mat4 quat_to_mat4(const Quat in_q) {
    ...
    float xy = x * y;   float wx = w * x;
    float xz = x * z;   float wy = w * y;
    float yz = y * z;   float wz = w * z;
